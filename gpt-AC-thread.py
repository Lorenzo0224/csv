...
Shapley_Value_record = [[[0, 0] for i in range(k + 1)] for j in range(20)]

##todo: new key is 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
##todo: set global parameters here
Total_m = 10 ##Maximum number of examples in QA
thread_num = 5

the_api_key = 'sk-C7b0zzvBflB37NOfEf779e7740A54d4aB06c3f5c3370E974'
...
openai.api_key = 'sk-C7b0zzvBflB37NOfEf779e7740A54d4aB06c3f5c3370E974'
...
dictionary = {}
# 包装函数，用于多线程执行并存储结果
def thread_function(n, samples, Uniform, AvgDiff):
    ...
    prefix = "When determining whether two Papers are the same, you should only focus on critical properties and overlook noisy factors.\n" + prefix + "Above are examples. Use domain knowledge of Papers to help understand the text and answer the following question in the format: Yes./No."
...
def thread_function2(n, samples, i):
    index_list = []
    index_list = random.choice(generate_subsets([i for i in range(n)], k))
    size = len(index_list)
    delta_budget, prefix = target_random_prompt(
    ...
    with dictionary_lock:
        dictionary[i] = (u, index_list, size)
...
def ActivatedContributions(Uniform=True, AvgDiff=False, MultiArmIden=False, k=5):
    result_list = []
    scoretest = []
    scorevali = []
    AvgDiff_klist = []
    ...
    samples = prompt_utils.get_validation_dataset(
        "outputs/"+dataset_name+"/validation/default/2k_0inst_0cb_random_200run_0dry/trial_0.feather",
        num_examples=20,
    ...
    while current_m < Total_m:
        ...
        for i in range(thread_num):
            thread = threading.Thread(target=thread_function2, args=(n, samples, i))
        ...
        for k, v in dictionary.items():
            for target in range(n):
                if (target in v[1]):
                    print(n / v[2], v[0])
                    Shapley_Value_record[target][v[2]][0] += (n / v[2] - 1) * v[0]
                if (target not in v[1] and v[2] < k):
                    Shapley_Value_record[target][v[2]][0] += -v[0]
                Shapley_Value_record[target][v[2]][1] += 1
            endtime = time.time()
            ...
            excel_file = 'k=' + str(k) + dataset_name + '_thread_neyman_prompt_SVnum=' + str(current_m) + '.xlsx'
...
def parse_args() -> argparse.Namespace:##setting default parameters for parser
    ...
    parser.add_argument(
        ...
        default="D:/GitHub/LLM-EM0001/fm_data_tasks-main/fm_data_tasks/data/datasets/entity_matching/structured/"+dataset_name,

...
def ActivatedContributions(Uniform=True, AvgDiff=False, MultiArmIden=False):
    ...
    samples = prompt_utils.get_validation_dataset(
        "outputs/"+dataset_name+"/validation/default/" + vali_name + "/trial_0.feather",
        num_examples=20,
        task=task,
    ...
    for current_m in range(Total_m):
        ...
        if (current_m % 5 == 0):
            ...
            if (MultiArmIden):
                method = 'mul'
            excel_file = 'k=' + str(k) + dataset_name +str(method) + 'gpt4omini'+'SVnum=' + str(current_m) + '.xlsx'
